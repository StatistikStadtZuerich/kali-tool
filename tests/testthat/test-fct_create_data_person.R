test_that("check data_person works", {
  ...
  expect_equal(create_data_person(filtered_data, row_to_be_selected)$Name,
               name_to_be_selected)
  # check columns: only 9
  expect_equal(ncol(create_data_person(filtered_data, row_to_be_selected)),
               9)
  # check rows: only 1
  expect_equal(nrow(create_data_person(filtered_data, row_to_be_selected)),
               1)
  # check data type
  expect_s3_class(create_data_person(filtered_data, row_to_be_selected),
                  "data.frame")
})
